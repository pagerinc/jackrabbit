...
const queue = (options) => {
    ...
    const onChannel = (err, chan) => {
        ...
        console.log('connecting queue:', emitter.name);

        emitter.once('ready', () => {

            consumers.forEach((consumer) => channel.consume(emitter.name, consumer.onMessage, consumer.opts, onConsume));
        });
    };
    ...
    console.log('creating queue:', options);
